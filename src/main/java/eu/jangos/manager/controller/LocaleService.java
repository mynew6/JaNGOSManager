...
public class LocaleService {     
    ...
    /**
     * Return the list of all locale available in the database.
     * @return A List of Locale corresponding to the content of the database.
     */
    public List<Locale> getAllLocale(){
        logger.debug("Returning all locale");
                
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createCriteria(Locale.class).addOrder(Order.asc("locale")).list();
